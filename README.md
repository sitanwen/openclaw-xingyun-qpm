# OpenClaw 模型 QPM 限流队列插件

这是一个 OpenClaw Provider 插件，用于控制模型的每分钟请求次数。

当最近 60 秒内的模型请求达到上限时，插件不会让接口返回 429，也不会中断当前 Agent 任务，而是让这次模型调用在 OpenClaw 内部排队等待。额度恢复后，插件会自动继续原来的模型请求，Agent 会从原来的执行位置继续运行。

默认策略为：最近连续 60 秒最多放行 17 次模型请求，并额外保留 300 毫秒的边界安全时间。接口限制为 18 QPM 时，插件主动少使用一个额度，用来降低时间边界误差以及其他进程共享同一密钥时产生 429 的风险。
...
- Node.js 22 或更高版本。
- 模型接口兼容 OpenAI Chat Completions。
...
```bash
export XINGYUN_API_KEY="你的 API Key"
export XINGYUN_BASE_URL="https://你的接口地址/v1"
...
| 变量 | 说明 |
| --- | --- |
| `XINGYUN_API_KEY` | 模型接口密钥。 |
| `XINGYUN_BASE_URL` | OpenAI 兼容接口地址。 |
| `XINGYUN_MODEL_ID` | 模型 ID。 |
| `OPENCLAW_GATEWAY_URL` | OpenClaw Gateway 地址。 |
| `OPENCLAW_GATEWAY_TOKEN` | 调用 `chat.inject` 时使用的 Gateway Token。 |
...
| 配置项 | 默认值 | 说明 |
| --- | ---: | --- |
| `baseUrl` | 环境变量中的地址 | 接口地址。 |
| `modelId` | 环境变量中的模型 ID | 使用的模型。 |
| `requestsPerMinute` | `17` | 最近连续 60 秒最多放行的请求数，建议不要直接设为 18。 |
| `windowMs` | `60000` | 滑动窗口长度，单位为毫秒。 |
| `safetyMs` | `300` | 窗口边界额外等待时间，单位为毫秒。 |
| `notify` | `true` | 排队时是否向当前对话发送提示。 |
| `notifyResumed` | `true` | 额度恢复时是否向当前对话发送提示。 |

## 三、限流原理

插件通过 OpenClaw Provider 的 `wrapStreamFn` 包装每一次真实模型调用。因此同一个 Agent 任务中的多轮模型请求、工具调用后的继续推理以及子 Agent 的模型请求，都会在发往模型接口之前先申请一个调用额度。

插件使用严格滑动窗口算法：

1. 保存最近 60 秒内已经放行的模型请求时间。
2. 新请求到达时，删除已经超过 60 秒的记录。
3. 如果窗口内请求数少于 17，立即放行。
4. 如果已经达到 17 次，就找到窗口内最早的一次请求。
5. 等到“最早请求时间 + 60 秒 + 300 毫秒安全余量”后，再重新检查并放行。
...
如果用户主动停止任务，OpenClaw 的 `AbortSignal` 会取消等待，不会在任务取消后继续调用模型。
...
1. 为当前固定版本的私有 `admin-http-rpc` 白名单加入 `chat.inject`。
2. 将通知模块改为通过 OpenClaw Gateway WebSocket 客户端调用 `chat.inject`。
3. 设置 `notify: false` 和 `notifyResumed: false`，只使用限流排队功能。

本插件目前按 OpenAI Chat Completions 兼容接口实现。如果使用 OpenAI Responses API 或 Anthropic Messages API，需要同步调整 Provider 的 `api` 类型和模型传输配置。
